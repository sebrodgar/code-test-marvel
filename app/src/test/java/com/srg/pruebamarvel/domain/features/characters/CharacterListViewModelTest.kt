...
import com.srg.pruebamarvel.presentation.features.characters.list.CharacterListViewModel
...
class CharacterListViewModelTest {
    ...
    @Before
    fun setUp() {
        viewModel = CharacterListViewModel(useCase, coroutineTestRule.testCoroutineDispatcher)
    }

    @Test
    fun `it should call service get characters use case and success interactios`() =
        coroutineTestRule.runBlockingTest {
            //given:
            val characters = listOf(CharacterApiModelFactory.createOne().toDomain())
            val characterId = Random.nextLong()
            useCase.stub { onBlocking { execute() } doReturn characters }
    ...
    @Test
    fun `it should call service get characters use case and error interactions`() =
        coroutineTestRule.runBlockingTest {
            //given:
            val error = NetworkException(409, APIErrorCode.CHARACTER_NOT_FOUND)
            val characterId = Random.nextLong()
            val params = GetCharacterUseCase.Params(characterId)
            useCase.stub { onBlocking { execute() } doAnswer { throw error } }
